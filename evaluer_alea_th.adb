...
procedure Evaluer_Alea_TH is
   ...
   is
      ...
      package th_ent_ent is
        new TH(T_Cle => Integer, T_Valeur => Integer, Capacite => 1000, hacher => Identite);
      use th_ent_ent;
   ...
   begin
      ...
      for i in 1..Taille loop
         begin
            ...
            Enregistrer(TH, random, La_Valeur(TH, random)+1);
         exception
            when Cle_Absente_Exception =>
               Enregistrer(TH, random, 1);
         end;
      end loop;
      frMin := Taille;
      frMax := 0 ;
      Pour_Chaque_CleVal(TH);
      Max := frMax;
      Min := frMin;
